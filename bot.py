...
# ========== УДАЛЯЕМ СТАРУЮ СЕССИЮ ПЕРЕД ЗАПУСКОМ ==========
...
# Создаем клиент с in_memory=True для bothost
app = Client(
    "line_anti_scam",
    api_id=API_ID,
    api_hash=API_HASH,
    bot_token=BOT_TOKEN,
    workers=NUM_WORKERS,
    in_memory=True  # Храним сессию в памяти для bothost
)
...
# ========== КОМАНДА ОФФТОП ==========
@app.on_message(command_filter(['оффтоп', 'offtop']))
async def offtop_command(app: Client, message: Message):
    """Команда оффтоп - удаляет сообщение и дает мут на 30 минут"""
    try:
        if message.chat.type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
            await message.reply('⚠️ Эта команда работает только в группах')
            return
        ...
        if message.reply_to_message:
            ...
            if target_status and target_status >= 2:
                await message.reply('⚠️ Нельзя замутить администратора базы')
                return
            ...
            try:
                ...
                await app.send_message(
                    message.chat.id,
                    f'⚠️ <b>Оффтоп наказан!</b>\n\n'
                    f'👤 <b>Пользователь:</b> {target_user.first_name}\n'
                    f'🆔 <b>ID:</b> <code>{user_id_target}</code>\n'
                    f'⏰ <b>Мут:</b> 30 минут\n'
                    f'👮 <b>Администратор:</b> {message.from_user.mention}\n'
                    f'📅 <b>Причина:</b> Оффтоп\n\n'
                    f'<i>Для оффтопа используйте: @LineReports</i>'
                )
                
            except ChatAdminRequired:
                await message.reply('❌ У бота нет прав администратора в этом чате')
            except Exception as e:
                logger.error(f"Ошибка мута: {e}")
                await message.reply(f'❌ Ошибка при муте: {str(e)}')
        else:
            await message.reply('⚠️ Ответьте на сообщение с оффтопом')
...
@app.on_message(command_filter(['del', 'delete', 'удалить', 'дел']))
async def delete_command(app: Client, message: Message):
    ...
    try:
        ...
        if message.reply_to_message:
            try:
                ...
                # Отправляем уведомление (можно закомментировать, если не нужно)
                # await app.send_message(
                #     message.chat.id,
                ...
                await message.reply('❌ У бота нет прав на удаление сообщений')
            except Exception as e:
                logger.error(f"Ошибка удаления сообщения: {e}")
                await message.reply(f'❌ Ошибка: {str(e)}')
        else:
            await message.reply('⚠️ Ответьте на сообщение, которое хотите удалить')
...
@app.on_message(command_filter(['unmute', 'размут', 'снятьмут']))
async def unmute_command(app: Client, message: Message):
    ...
    try:
        ...
        if message.reply_to_message:
            ...
            # Проверяем, не пытаемся ли размутить администратора
            target_status = check_status(user_id_target)
            if target_status and target_status >= 2:
                await message.reply('⚠️ Администратор и так не может быть замучен')
                return
...
# ========== КОМАНДА BAN (БАН) ==========
@app.on_message(command_filter(['ban', 'бан', 'забанить']))
async def ban_command(app: Client, message: Message):
    ...
    try:
        ...
        if message.reply_to_message:
            ...
            target_status = check_status(user_id_target)
            if target_status and target_status >= 2:
                await message.reply('⚠️ Нельзя забанить администратора базы')
                return
            
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("1 день", callback_data=f"ban_1440_{user_id_target}"),
                    InlineKeyboardButton("3 дня", callback_data=f"ban_4320_{user_id_target}"),
                    InlineKeyboardButton("7 дней", callback_data=f"ban_10080_{user_id_target}")
                ],
                [
                    InlineKeyboardButton("30 дней", callback_data=f"ban_43200_{user_id_target}"),
                    InlineKeyboardButton("Навсегда", callback_data=f"ban_permanent_{user_id_target}")
                ]
            ])
            
            await message.reply(
                f'⏰ Выберите время бана для пользователя {target_user.first_name}:',
                reply_markup=keyboard
            )
        else:
...
@app.on_message(command_filter(['unban', 'разбан', 'снятьбан']))
async def unban_command(app: Client, message: Message):
    ...
    try:
        ...
        if message.reply_to_message:
            ...
            try:
                await app.unban_chat_member(message.chat.id, user_id_target)
            ...
            # Если не ответом на сообщение, пробуем через аргументы
            text = message.text or ""
            # Удаляем префикс команды
            for prefix in ['/', '!', '.', '-']:
                if text.startswith(f"{prefix}unban"):
                    ...
                    break
                elif text.startswith(f"{prefix}снятьбан"):
                    text = text[len(f"{prefix}снятьбан"):].strip()
                    break
            
            if not text:
                await message.reply('⚠️ Ответьте на сообщение пользователя или укажите ID/@username')
                return
            ...
            if target_input.isdigit():
                user_id_target = int(target_input)
            elif target_input.startswith('@'):
                try:
                    user_obj = await app.get_users(target_input)
                    user_id_target = user_obj.id
                except:
                    await message.reply('⚠️ Пользователь не найден')
                    return
            else:
                await message.reply('⚠️ Неверный формат. Используйте ID или @username')
                return
...
async def ban_time_callback(app: Client, callback_query: CallbackQuery):
    """Обработка бана"""
    try:
        data = callback_query.data
        parts = data.split('_')
        ...
        target_status = check_status(target_user_id)
        if target_status and target_status >= 2:
            await callback_query.answer("⚠️ Нельзя забанить администратора", show_alert=True)
            return
        
        if time_str == "permanent":
            ban_until = datetime.now() + timedelta(days=31)
            time_text = "навсегда"
        else:
            minutes = int(time_str)
        ...
        try:
            ...
            await callback_query.edit_message_text(
                ...
                f'⏰ <b>Время:</b> {time_text}\n'
                f'👮 <b>Администратор:</b> {callback_query.from_user.mention}\n\n'
                f'<i>Для разбана используйте /unban</i>'
            )
        ...
        logger.error(f"Ошибка в ban_time_callback: {e}")
        await callback_query.answer("❌ Произошла ошибка", show_alert=True)
...
if __name__ == "__main__":
    try:
        print("=" * 60)
        print("🚀 ЗАПУСК LINE ANTI SCAM BOT НА BOTHOST.RU")
        print("=" * 60)
        
        # Убедимся что сессия удалена
        if os.path.exists("line_anti_scam.session"):
            os.remove("line_anti_scam.session")
